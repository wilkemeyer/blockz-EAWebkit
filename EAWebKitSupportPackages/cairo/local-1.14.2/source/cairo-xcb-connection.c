					  ...
					  const xcb_render_query_pict_formats_reply_t *formats)
{
	     ...
	     xcb_render_pictdepth_next (&depths))
	{
		...
		f = cairo_malloc (sizeof (cairo_xcb_xrender_format_t));
		if (unlikely (f == NULL))
...
_device_destroy (void *device)
{
    ...
    cairo_free (connection->subpixel_orders);
    cairo_free (connection);
}
...
_cairo_xcb_connection_get (xcb_connection_t *xcb_connection)
{
    ...
    connection = cairo_malloc (sizeof (cairo_xcb_connection_t));
    ...
    connection->subpixel_orders = cairo_calloc (connection->root->roots_len, sizeof(*connection->subpixel_orders));
...
_cairo_xcb_connection_get_xid (cairo_xcb_connection_t *connection)
{
	...
	_cairo_freepool_cairo_free (&connection->xid_pool, cache);
    } else {
