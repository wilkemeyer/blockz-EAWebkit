					       ...
					       double			 height)
{
    ...
    surface = cairo_malloc (sizeof (cairo_pdf_surface_t));
				       ...
				       const cairo_rectangle_int_t  *extents)
{
    ...
    group = cairo_calloc (1, sizeof (cairo_pdf_smask_group_t));
...
_cairo_pdf_smask_group_destroy (cairo_pdf_smask_group_t *group)
{
    ...
    cairo_free (group->utf8);
    cairo_free (group->glyphs);
    cairo_free (group->clusters);
				       ...
				       cairo_rectangle_int_t        *source_extents)
{
	...
	*source_extents = surface_entry->extents;
	...
	if (surface_key.unique_id && surface_key.unique_id_length > 0) {
	    unique_id = _cairo_cairo_malloc (surface_key.unique_id_length);
	    if (unique_id == NULL) {
		status = _cairo_error (CAIRO_STATUS_NO_MEMORY);
		goto release_source;
	    }
...
release_source:
    ...
    surface_entry = cairo_malloc (sizeof (cairo_pdf_source_surface_entry_t));
...
fail2:
    cairo_free (surface_entry);

fail1:
    cairo_free (unique_id);
...
_cairo_pdf_source_surface_entry_pluck (void *entry, void *closure)
{
    ...
    cairo_free (surface_entry->unique_id);
					...
					cairo_pdf_jbig2_global_t **entry)
{
	...
	*entry = (cairo_pdf_jbig2_global_t *) _cairo_array_index (&surface->jbig2_global, i);
    ...
    global.id = cairo_malloc(global_id_length);
...
_utf8_to_pdf_string (const char *utf8, char **str_out)
{
	...
	str = cairo_malloc (utf16_len*4 + 7);
	if (str == NULL) {
	    cairo_free (utf16);
	    return _cairo_error (CAIRO_STATUS_NO_MEMORY);
	}
				     ...
				     const cairo_clip_t		*clip)
{
	...
	if (utf8_len) {
	    group->utf8 = cairo_malloc (utf8_len);
