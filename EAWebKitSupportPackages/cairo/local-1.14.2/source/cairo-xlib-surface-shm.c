...
static void _cairo_xlib_display_sync (cairo_xlib_display_t *display)
{
	...
	_cairo_mempool_cairo_free (&info->pool->mem, info->mem);
			   ...
			   void **ptr, unsigned long *last_request)
{
	...
	*last_request = info->last_request;
	...
	_pqueue_pop (&display->shm->info);
	_cairo_mempool_cairo_free (&pool->mem, info->mem);
	cairo_free (info);
			    ...
			    size_t size, void **ptr)
{
    ...
    pool = cairo_malloc (sizeof (cairo_xlib_shm_t));
    if (pool == NULL)
	return NULL;
			     ...
			     size_t size, cairo_bool_t will_sync)
{
    ...
    info = cairo_malloc (sizeof (*info));
    if (info == NULL) {
	_cairo_mempool_cairo_free (&pool->mem, mem);
	return NULL;
    }
...
_cairo_xlib_shm_surface_finish (void *abstract_surface)
{
	...
	_cairo_mempool_cairo_free (&shm->info->pool->mem, shm->info->mem);
	cairo_free (shm->info);
