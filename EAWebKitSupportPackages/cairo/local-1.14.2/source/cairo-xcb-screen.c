...
_pattern_cache_entry_destroy (void *closure)
{
    ...
    _cairo_freelist_cairo_free (&entry->screen->pattern_cache_entry_freelist, entry);
}
		       ...
		       xcb_screen_t *xcb_screen)
{
    ...
    screen = cairo_malloc (sizeof (cairo_xcb_screen_t));
					...
					cairo_surface_t *picture)
{
    ...
    if (unlikely (status)) {
	_cairo_freelist_cairo_free (&screen->pattern_cache_entry_freelist, entry);
	return status;
    }
