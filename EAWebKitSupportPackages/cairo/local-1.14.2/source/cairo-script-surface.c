		 ...
		 unsigned long *id)
{
    ...
    bb = cairo_malloc (sizeof (struct _bitmap));
    if (unlikely (bb == NULL))
	    ...
	    cairo_bool_t force)
{
	...
	cairo_free (surface->cr.current_style.dash);
...
_cairo_script_surface_finish (void *abstract_surface)
{
	...
	if (! cairo_list_is_empty (&surface->operand.link)) {
		...
		struct deferred_finish *link = cairo_malloc (sizeof (*link));
		if (link == NULL) {
			...
			cairo_script_font_t **font_out)
{
    ...
    font_private = cairo_malloc (sizeof (cairo_script_font_t));
...
_cairo_script_implicit_context_reset (cairo_script_implicit_context_t *cr)
{
    cairo_free (cr->current_style.dash);
				       ...
				       cairo_surface_t *passthrough)
{
    ...
    surface = cairo_malloc (sizeof (cairo_script_surface_t));
    if (unlikely (surface == NULL))
...
_cairo_script_context_create_internal (cairo_output_stream_t *stream)
{
    ...
    ctx = cairo_malloc (sizeof (cairo_script_context_t));
    if (unlikely (ctx == NULL))
