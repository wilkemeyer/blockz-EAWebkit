...
static void
_cairo_cogl_journal_cairo_free (cairo_cogl_surface_t *surface)
{
    GList *l;
    ...
    g_queue_cairo_free (surface->journal);
...
_cairo_cogl_surface_finish (void *abstract_surface)
{
    ...
    if (surface->journal)
	_cairo_cogl_journal_cairo_free (surface);
				  ...
				  double tolerance)
{
    ...
    meta = cairo_calloc (1, sizeof (cairo_cogl_path_stroke_meta_t));
    if (!meta)
	goto BAIL;
    CAIRO_REFERENCE_COUNT_INIT (&meta->ref_count, 1);
    meta->cache_entry.hash = hash;
    meta->counter = 0;
    meta_path = cairo_malloc (sizeof (cairo_path_fixed_t));
    if (!meta_path)
	goto BAIL;
...
_cairo_cogl_get_path_fill_meta (cairo_cogl_surface_t *surface)
{
    ...
    meta = cairo_calloc (1, sizeof (cairo_cogl_path_fill_meta_t));
    if (!meta)
	goto BAIL;
    meta->cache_entry.hash = hash;
    meta->counter = 0;
				 ...
				 CoglTexture *texture)
{
    ...
    surface = cairo_malloc (sizeof (cairo_cogl_surface_t));
