...
_cairo_cogl_linear_gradient_destroy (cairo_cogl_linear_gradient_t *gradient)
{
    ...
    g_list_cairo_free (gradient->textures);
				 ...
				 cairo_cogl_linear_gradient_t **gradient_out)
{
    ...
    if (!gradient) {
	gradient = cairo_malloc (sizeof (cairo_cogl_linear_gradient_t) +
    ...
    entry = cairo_malloc (sizeof (cairo_cogl_linear_texture_entry_t));
    if (!entry) {
