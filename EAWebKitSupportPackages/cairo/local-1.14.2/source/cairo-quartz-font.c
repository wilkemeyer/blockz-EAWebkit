					...
					cairo_font_face_t      **font_face)
{
    ...
    full_name = cairo_malloc (strlen (family) + 64); // give us a bit of room to tack on Bold, Oblique, etc.
					    ...
					    cairo_scaled_font_t **font_out)
{
    ...
    font = cairo_malloc(sizeof(cairo_quartz_scaled_font_t));
    if (font == NULL)
	return _cairo_error (CAIRO_STATUS_NO_MEMORY);
...
cairo_quartz_font_face_create_for_cgfont (CGFontRef font)
{
    ...
    font_face = cairo_malloc (sizeof (cairo_quartz_font_face_t));
    if (!font_face) {
