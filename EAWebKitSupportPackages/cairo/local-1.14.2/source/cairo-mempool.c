...
static void *
buddy_cairo_malloc (cairo_mempool_t *pool, int bits)
{
    size_t past, offset;
    struct _cairo_memblock *block;
    int b;
		      ...
		      int min_bits, int num_sizes)
{
    ...
    pool->blocks = cairo_calloc (num_blocks, sizeof (struct _cairo_memblock));
    if (pool->blocks == NULL)
    ...
    pool->map = cairo_malloc ((num_blocks + 7) >> 3);
    if (pool->map == NULL) {
	cairo_free (pool->blocks);
	return _cairo_error (CAIRO_STATUS_NO_MEMORY);
    }
...
_cairo_mempool_alloc (cairo_mempool_t *pool, size_t bytes)
{
    ...
    return buddy_cairo_malloc (pool, bits);
}

void
_cairo_mempool_cairo_free (cairo_mempool_t *pool, void *storage)
{
    size_t block_offset;
    struct _cairo_memblock *block;
