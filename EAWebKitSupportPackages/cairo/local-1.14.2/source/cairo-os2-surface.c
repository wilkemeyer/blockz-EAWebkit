 ...
 */
void _buffer_cairo_free (void *buffer)
{
#ifdef OS2_USE_PLATFORM_ALLOC
    DosFreeMem (buffer);
#else
                          ...
                          int height)
{
    ...
    local_os2_surface = (cairo_os2_surface_t *) cairo_malloc (sizeof (cairo_os2_surface_t));
                            ...
                            int              timeout)
{
    ...
    if (pNewImageSurface->base.status) {
         ...
         * Everything remains the same!
         */
        _buffer_cairo_free (pchNewPixels);
        return _cairo_error (CAIRO_STATUS_NO_MEMORY);
    }
