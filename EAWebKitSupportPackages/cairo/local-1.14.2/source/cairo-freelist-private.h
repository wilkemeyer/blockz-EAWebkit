 ...
 * nodes, a new one will be allocated using cairo_malloc().  The caller is
 * responsible for calling _cairo_freelist_cairo_free() or cairo_free() on the
 * returned node.  Returns %NULL on memory allocation error. */
cairo_private void *
_cairo_freelist_alloc (cairo_freelist_t *freelist);

/* Allocate a new node from the freelist.  If the freelist contains no
 * nodes, a new one will be allocated using cairo_calloc().  The caller is
 * responsible for calling _cairo_freelist_cairo_free() or cairo_free() on the
 * returned node.  Returns %NULL on memory allocation error. */
cairo_private void *
_cairo_freelist_cairo_calloc (cairo_freelist_t *freelist);
...
cairo_private void
_cairo_freelist_cairo_free (cairo_freelist_t *freelist, void *node);
...
static inline void
_cairo_freepool_cairo_free (cairo_freepool_t *freepool, void *ptr)
{
