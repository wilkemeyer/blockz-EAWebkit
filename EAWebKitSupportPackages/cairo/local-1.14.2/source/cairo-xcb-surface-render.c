			   ...
			   int width, int height)
{
    ...
    surface = cairo_malloc (sizeof (cairo_xcb_picture_t));
...
_cairo_xcb_font_destroy (cairo_xcb_font_t *font)
{
	...
	cairo_free (info->pending_free_glyphs);
    }
			...
			cairo_scaled_font_t  *font)
{
    ...
    priv = cairo_malloc (sizeof (cairo_xcb_font_t));
		       ...
		       cairo_scaled_font_t  *font)
{
	...
	if (to_free == NULL) {
	    to_free = cairo_malloc (sizeof (cairo_xcb_font_glyphset_free_glyphs_t));
		    ...
		    xcb_render_pictformat_t mask_format)
{
	...
	buf = cairo_malloc (estimated_req_size);
