...
_cairo_int64_divrem (cairo_int64_t num, cairo_int64_t den)
{
	...
	qr.rem = uqr.rem;
	//+EAWebKitChange
	//11/7/2011 - SNC does not like cast to cairo_int64_t presumably because the type is already cairo_int64_t
#if defined(CS_UNDEFINED_STRING) || defined(EA_PLATFORM_CAPILANO)
	if (num_neg != den_neg)
