 ...
 * Copyright © 2011, 2013 Electronic Arts, Inc.
 *
...
_cairo_pattern_reset_solid_pattern_cache (void)
{
    ...
    for (i = 0; i < MIN (ARRAY_LENGTH (solid_pattern_cache.patterns), solid_pattern_cache.size); i++) {
        ...
        if (solid_pattern_cache.patterns[i])
            cairo_free (solid_pattern_cache.patterns[i]);
        //-EAWebKitChange
	solid_pattern_cache.patterns[i] = NULL;
    }
...
#define PIXMAN_MAX_INT ((pixman_fixed_1 >> 1) - pixman_fixed_e) /* need to ensure deltas also fit */
	...
	*out = &image->base;
    ...
    //+EAWebKitChange - Integrated the _cairo_matrix_to_pixman_matrix from Cairo 10.2.1 to fix a shadow box problem. 
    //6/26/2013
    _cairo_matrix_to_pixman_matrix (&matrix, &pixman_transform, width/2., height/2.);
    //-EAWebKitChange
